...
public class CustomQueries {
    ...
    public Long getUserClientServiceByCredential(String username, Long clientServiceId) {
    	
    	if (username == null || username.isEmpty() || clientServiceId == null) {
            System.out.println("user:" + username + " or clientServiceId:" + clientServiceId + " is null or empty in IsUserPasswordUpdated");
            return null;
    	}
...
//            String queryStr = ""
//            		+ "select "
//            		+ "	case when timestampdiff(day, COALESCE(ucs.last_loggedin_at, now()), :loginTimeStamp) < css.inactivity_period then 0 "
//            		+ "	else timestampdiff(day, COALESCE(ucs.last_loggedin_at, now()), :loginTimeStamp) end "
//            		+ "from user_client_service ucs "
...
//            		+ "        when timestampdiff( "
//            		+ "            day, "
//            		+ "            COALESCE((select max(ucs.last_loggedin_at) from user_client_service ucs inner join client_service cs on cs.id = ucs.client_service_id where ucs.user_id = 1 and cs.client_id = 2), now()), "
//            		+ "            :loginTimeStamp "
//            		+ "        ) < (select css.inactivity_period from client_settings css where css.client_id = 2) "
//            		+ "        then 0 "
//            		+ "        else timestampdiff( "
//            		+ "            day, "
//            		+ "            COALESCE((select max(ucs.last_loggedin_at) from user_client_service ucs inner join client_service cs on cs.id = ucs.client_service_id where ucs.user_id = 1 and cs.client_id = 2), now()), "
...
//            		+ "		day, "
//            		+ "		COALESCE((select max(ucs.last_loggedin_at) from user_client_service ucs where ucs.user_id = :userId and ucs.client_id = :clientId), now()), :loginTimeStamp"
//            		+ "	) < (select inactivity_period from client_settings cs where cs.client_id = :clientId) then 0 "
//            		+ "	else timestampdiff(day, COALESCE((select max(ucs.last_loggedin_at) from user_client_service ucs where ucs.user_id = :userId and ucs.client_id = :clientId), now()), :loginTimeStamp) end ";
